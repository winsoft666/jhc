...
#define SHA2_UNPACK64(x, str)              \
    {                                      \
        ...
        *((str) + 4) = (SHA512::uint8)((x) >> 24); \
        *((str) + 3) = (SHA512::uint8)((x) >> 32); \
        *((str) + 2) = (SHA512::uint8)((x) >> 40); \
        *((str) + 1) = (SHA512::uint8)((x) >> 48); \
...
void SHA512::transform(const unsigned char* message, unsigned int block_nb) {
    ...
    for (i = 0; i < (int)block_nb; i++) {
        ...
        for (j = 0; j < 80; j++) {
            t1 = wv[7] + SHA512_F2(wv[4]) + SHA2_CH(wv[4], wv[5], wv[6]) + getSha512K(j) + w[j];
